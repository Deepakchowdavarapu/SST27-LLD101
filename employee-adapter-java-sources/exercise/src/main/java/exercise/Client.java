...
public class Client {
  public static void main(String[] args) {
    ...
    for (EmployeeCSV row : csvRows) all.add(new EmployeeCSVAdapter(row));
    for (EmployeeDB row : dbRows) all.add(new EmployeeDBAdapter(row));
    for (EmployeeLDAP row : ldapRows) all.add(new EmployeeLDAPAdapter(row));
